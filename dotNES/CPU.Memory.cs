...
namespace dotNES
{
    sealed partial class CPU
    {
        ...
        {
            var def = _opcodeDefs[_currentOpcode];
            switch (def.Mode)
            {
                case Immediate:
                    return PC++;
        ...
        {
            if (_opcodeDefs[_currentOpcode].Mode == Direct) return _rmwValue = A;
        ...
        {
            if (_opcodeDefs[_currentOpcode].Mode == Direct) A = val;
            else
            {
                if (_currentMemoryAddress == null) _currentMemoryAddress = _Address();
                if (_opcodeDefs[_currentOpcode].RMW)
